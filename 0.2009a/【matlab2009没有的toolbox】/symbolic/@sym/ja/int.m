...
% 不定積分を出力します。S は、シンボリックオブジェクト(行列またはスカラ)です。
% S が定数ならば、積分は、'x' について計算されます。
% 
% INT(S, v) は、v について S の不定積分を出力します。
% v は、スカラシンボリックオブジェクトです。
% INT(S, a, b) は、シンボリック変数について、a から b までの S の定積分を
% 出力します。a と b は、倍精度のスカラ、または、シンボリックなスカラです。
...
%     int([exp(t),exp(alpha*t)]) は、[exp(t), 1/alpha*exp(alpha*t)] を出力します。
