...
%
% 陰的に、関数 f=f(x,y) を定義します。EZPLOT(f) は、デフォルトの領域 
% -2*pi < x  < 2*pi, -2*pi < y < 2*pi で、f(x,y) = 0 をプロットします。
%
% EZPLOT(f, [xmin,xmax,ymin,ymax]) は、xmin < x < xmax,  ymin < y < ymax で、
% f(x,y) = 0 をプロットします。
% 
% EZPLOT(f, [a,b])は、a < x < b, a < y < b で、f(x,y) = 0 をプロットします。
% f が、変数u,vの関数である場合は、領域の終了点a,bは、アルファベット順に
% ソートされます。たとえば、EZPLOT(u^2 - v^2 - 1,[-3,2,-2,3]) は、
% -3 < u < 2, -2 < v < 3 で、u^2 - v^2 - 1 = 0 をプロットします。
%
% EZPLOT(x,y) は、デフォルトの領域 0 < t < 2*pi で、パラメトリックに
% 定義された平面曲線 x=x(t), y=y(t) をプロットします。
% EZPLOT(x,y, [tmin,tmax]) は、tmin < t < tmax で、x=x(t), y=y(t) を
% プロットします。
%
% EZPLOT(f, [a,b], FIG), EZPLOT(f, [xmin,xmax,ymin,ymax], FIG)、または、
% EZPLOT(x,y, [tmin,tmax], FIG) は、figure ウィンドウFIGで指定された
% 領域で、与えられた関数をプロットします。
%
% 例題：
%     syms x y t
