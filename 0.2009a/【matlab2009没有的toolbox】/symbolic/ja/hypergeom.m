% HYPERGEOM   ガウスの超幾何関数
%
% HYPERGEOM(N, D, Z) は、一般化超幾何関数 F(N, D, Z) で、jFk で定義される 
% Barnes 拡張超幾何関数としても知られています。ここで、j = length(N) で、
% k = length(D) です。スカラ a, b, c に対して、HYPERGEOM([a,b], c, z) は、
% ガウス超幾何関数 2F1(a, b;c;z) になります。
% 
% 正式なベキ級数の定義は、つぎのようになります。
% 
%    hypergeom(N,D,z) = sum(k=0:inf, (C(N,k)/C(D,k))*z^k/k!) 
% 
% ここで、C(V,k) = prod(i=1:length(V), gamma(V(i)+k)/gamma(V(i))) と
% なります。最初の2つの引数のどちらも、単一関数計算に対する係数パラメータを
% 与えるベクトルです。3番目の引数がベクトルの場合、関数は、区分的に評価
% されます。結果は、すべての引数が数値の場合、数値になり、引数のいずれかが
% 数式の場合、結果は数式になります。Abramowitz と Stegun 著の Handbook 
